...
import com.uvg.laboratorio10.data.network.dto.mapToLocationModel
...
class LocationRepository(context: Context) {
    ...
    //pasar el Api para las locations
    ...
    //Suspend function para recibir las locaciones del API
    suspend fun getAllLocationsFromApi(): List<LocationEntity>{
        val response = api.getLocations()
        return response.results.map { it.mapToLocationModel()}
    }
}
